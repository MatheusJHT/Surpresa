...
function PresentEditor({ present, onSaved }: { present: AdminPresent; onSaved: () => Promise<void> }) {
  ...
  useEffect(() => {
    setForm({ title: present.title, question: present.question, answer: '', password: '', success_message: present.success_message, riddle: present.riddle, photo_url: present.photo_url ?? '', hint: present.hint ?? '', normalize_accents: present.normalize_accents, ignore_punctuation: present.ignore_punctuation, is_active: present.is_active })
  }, [present])
